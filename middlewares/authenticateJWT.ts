...
const authenticateJWT = async (req: userInfo, res: Response, next: NextFunction) => {
  try {
    const [accessToken, refreshToken] = req.headers?['authorization'] ? req.headers['authorization']!.split(',') : undefined;
...
exports.authenticateJWT = async (req:Request , res:Response, next:NextFunction) => {
  try {
    const [accessToken, refreshToken] = req.headers?['authorization'] ? req.headers['authorization']!.split(',') : undefined;
