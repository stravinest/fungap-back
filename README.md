...
services:
 ...
 ### 도커 스웜을 사용한 컨테이너 관리
  ...
  저희 코드는 .env파일을 app.ts 파일과 같은 위치에 두었고 서비스 생성 문에서 mount 옵션으로 usr/src/app 의 위치를 공유 하도록 설정하였습니다.<br>
  ...
  트러블 슈팅은 다음과 같습니다.<br>
  첫번째 이미지파일이 문제 있는지 확인하였습니다 -> 로컬에서 이미지만 받아서 실행시 문제 없음<br>
  두번째 롤링 업데이트에 이상이 있는지 확인하였습니다. ->  업데이트시에 로그를 받아서 확인 시에 문제가 없음<br>
  세번째 실질적으로 코드가 수정이 되었는지 확인하였습니다 -> 컨테이너 안으로 직접들어가 코드를 뜯어보니 수정이 안되어 있었음!!<br>
  ...
  기본적인 노드별 리소스 사용량과 전체적인 리소스 사용량을 모니터링 할 수 있습니다. 
  
  ### 부하테스트 
  배포용 서버와 똑같은 테스트용 서버를 만들어서 아파치jmeter로 부하를 측정하였습니다.<br>
  똑같은 환경을 구축하는데 20분도 걸리지 않았습니다. 할일은 docker와 docker compose를 설치하고 yml파일을 생성해서<br>
  돌려주고 도커스웜으로 조인만 해주면 되었습니다. <br>
  
  부하테스트 시나리오
  1000명 부터 100명씩 10초간의 시간을 두고 1 번씩 access한다.
  시간을 10초로 준이유: 너무 짧은 시간은 그래프에 잘 나타 나지 않아서 최대한 유의미한 그래프를 뽑기 위한 최소한의 시간이라 판단
